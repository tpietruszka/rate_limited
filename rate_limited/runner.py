...
validators_type = Optional[Union[validation_function_type, List[validation_function_type]]]
...
class Runner:
    def __init__(
        ...
        max_retries: int = 5,
        validators: validators_type = None,
        ...
        self.validation_function = self.get_validation_function(validators)
    ...
    def get_validation_function(
        self, validators: validators_type
    ) -> Optional[validation_function_type]:
        """
        Composes the validation function from the passed validators argument
        If None, or callable, just passthrough
        If its a list of callables, combine them with AND
        """
        if validators is None:
            return None
        if callable(validators):
            return self.validation_function
        if isinstance(self.validation_function, list):
            return lambda result: all(f(result) for f in self.validation_function)
        raise ValueError("Validation function must be a callable or a list of callables")
